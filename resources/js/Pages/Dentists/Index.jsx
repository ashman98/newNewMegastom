...
import AddDentistsForm from "@/components/Dentists/AddDentistsForm.jsx";
import DentistsTable from "@/components/DentistsTable.jsx";
...
const DentistsIndex = ({ dentists}) => {
    const [open, setOpen] = useState(false);
    const [filter, setFilter] = useState({});
    const [addDentistLoading, setAddDentistLoading] = useState(false);
    const [onDentistAdd, setOnDentistAdd] = useState(false);
    const { width } = useWindowSize();
    const toggleModal = () => {
        if (!addDentistLoading){
            setOpen((cur) => !cur);
        }
    }

    const toggleOnDentistAdd = () => setOnDentistAdd((cur) => !cur);
                ...
                <div className="patients-container">
                    <div className='flex justify-between mb-3'>
                        <Button color='gray' variant='gradient' className='flex gap-3 items-center'
                                onClick={toggleModal} loading={addDentistLoading}>
                    ...
                    <GenericModal
                        open={open}
                        onClose={toggleModal}
                        title="Ավելացնել նոր բժիշկ"
                        footer={false}
                    >
                        <AddDentistsForm toggleOnDentistAdd={toggleOnDentistAdd} toggleModal={toggleModal}/>
                    </GenericModal>
                    {/*<button onClick={addNewProduct} className="add-product-button">*/}
                    {/*    Добавить продукт*/}
                    {/*</button>*/}

                    <DentistsTable filter={filter} onDentistAdd={onDentistAdd}/>
                </div>
            </div>
        </AuthenticatedLayout>
    );
